...
mod_1_wfl <- workflow() %>%
  add_model(linear_reg() %>%
              set_engine("lm") %>%
              set_mode("regression")) %>%
  add_recipe(recipe(formula = reg_chg ~ treatment + poverty_n,
                    data = week_10_train))
```
...
mod_2_wfl <- workflow() %>%
  add_model(linear_reg() %>%
              set_engine("lm") %>%
              set_mode("regression")) %>%
  add_recipe(recipe(formula = reg_chg ~ treatment + poverty_n + county + block_number,
                    data = week_10_train) %>% 
               step_poly(poverty_n, degree = 10))
...
It looks like we over-fit so we had an inflated idea of how good our model actually was.
